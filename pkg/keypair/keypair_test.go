...
func TestKeyPair_Address(t *testing.T) {
	...
	assert.Equal(t,
		Address("TCGQQKN5HED66OQ67Z2F7GGWZ66DWVBFJUW6F5WC"), KeyPair{Public: pub}.Address(core.Testnet))
}

func TestAddress_PrettyString(t *testing.T) {
	_, err := Address("TCGQQKN5HED66OQ67Z2F7GWC").PrettyString()
	assert.Error(t, err)

	ps, _ := Address("TCGQQKN5HED66OQ67Z2F7GGWZ66DWVBFJUW6F5WC").PrettyString()
	assert.Equal(t,
